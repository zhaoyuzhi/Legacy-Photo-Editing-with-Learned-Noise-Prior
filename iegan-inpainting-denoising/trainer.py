...
def Trainer(opt):
    ...
    def save_model(net, epoch, opt):
        ...
        model_name = 'GrayInpainting_epoch%d_batchsize%d_noise%.2f.pth' % (epoch, opt.batch_size, opt.noise_level)
    ...
    for epoch in range(opt.epochs):
        for batch_idx, (masked_img, img) in enumerate(dataloader):
            ...
            out = generator(masked_img)                                     # out: [B, 1, 256, 256]

            # Mask L1 Loss
            MaskL1Loss = L1Loss(out, img)
        ...
        utils.sample(masked_img, out, img, opt.sample_path, (batches_done + 1))
...
def Trainer_GAN(opt):
    ...
    for epoch in range(opt.epochs):
        for batch_idx, (masked_img, img) in enumerate(dataloader):
            ...
            # LSGAN vectors
            valid = Tensor(np.ones((masked_img.shape[0], 1, 8, 8)))
            fake = Tensor(np.zeros((masked_img.shape[0], 1, 8, 8)))
            ...
            # Fake samples
            fake_scalar = discriminator(out.detach())
            # True samples
            ...
            # GAN Loss
            fake_scalar = discriminator(out)
            MaskGAN_Loss = MSELoss(fake_scalar, valid)

            # Get the deep semantic feature maps, and compute Perceptual Loss
            out_3c = torch.cat((out, out, out), 1)
            grayscale_3c = torch.cat((img, img, img), 1)
        ...
        # Save the model
        save_model(generator, (epoch + 1), opt)
        utils.sample(masked_img, out, img, opt.sample_path, (epoch + 1))
