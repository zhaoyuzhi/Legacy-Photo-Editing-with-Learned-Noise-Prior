...
def sample(masked_img, out, img, save_folder, epoch):
    # to cpu
    masked_img = masked_img[0, :, :, :].data.cpu().numpy().transpose(1, 2, 0)                   # 256 * 256 * 1
    out = out[0, :, :, :].data.cpu().numpy().transpose(1, 2, 0)                                 # 256 * 256 * 1
    img = img[0, :, :, :].data.cpu().numpy().transpose(1, 2, 0)                                 # 256 * 256 * 1
    # process
    ...
    img = np.concatenate((img, img, img), axis = 2)                                             # 256 * 256 * 3 (√)
    img = (img * 255).astype(np.uint8)
    # save
    saveimg = np.concatenate((masked_img, out, img), axis = 1)
